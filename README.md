...
核的规范化定义：<br/>
![](http://latex.codecogs.com/gif.latex?K(x_{i},x_{j})=\phi(x_{i})^{T}\phi(x_{j}))<br/><br/>
...
所以验证了：<br/>
![](http://latex.codecogs.com/gif.latex?K(x,z)=\phi(x)^{T}\phi(z))<br/>
关于核的具体内容，SVM有更详细的阐述。
...
#### AdaBoost算法
推进法主要用于减少一个估计器的方差，会创建同类估计器的集成，可用于分类任务和回归任务。<br/>
AdaBoost算法是推进法的一个典例。
...
当算法达到完美性能或经过一定次数的迭代后，就会停止，给出基础估计预测的权重和。

scikit-learn实现的推进元估计器：
- AdaBoostClassifier
- AdaBoostRegressor
...
- 虽然一定能收敛出解决方案，但无论停止条件如何设置，都不一定能收敛出全局最优解
...
PCA将一系列可能相关联的高维变量减少为一系列被称为主成分的低维度线性不相关(彼此正交)合成变量(将数据投射到一个低维子空间)，这些低维度数据会尽可能多地保存原始数据的方差。
