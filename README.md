...
贝叶斯分类器被称为生成式分类器的原因是：它仅仅是对输入的训练数据集进行了若干“计数”操作。

朴素贝叶斯是一种简单且高效的算法。<br/>
其高效的原因是：它通过单独查看每个特征来学习参数，并从每个特征中手机简单的类别数据，并从每个特征中收集简单的类别统计数据。

朴素贝叶斯的必要说明：
- 朴素贝叶斯的“朴素”体现在它简单的认为各个特征之间都是彼此独立的，这样可以大大简化贝叶斯算法的计算。
...
机器学习常见的降维方法：
- L1惩罚项的模型
- 主成分分析法(PCA)：一种无监督的降维方法，将原始样本映射到维度更低的样本空间中去
- 线性判别分析法(LDA)：一种有监督的降维方法，将原始样本映射到维度更低的样本空间中去
